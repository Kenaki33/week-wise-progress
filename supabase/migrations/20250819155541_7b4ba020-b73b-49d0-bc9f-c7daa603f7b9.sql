...
BEGIN
  ...
  IF pct < 0.67 AND NEW.habit_name IS DISTINCT FROM prev_name THEN
    RAISE EXCEPTION 'Nie można zmienić nawyku w tygodniu %, ponieważ w poprzednim tygodniu (%, ukończono % z % dni = %%) nie osiągnięto wymaganego progu 67%%. Kontynuuj ten sam nawyk: "%".',
      NEW.week_key, prev_key, completed_count, countable_days, round(pct*100, 1), prev_name
